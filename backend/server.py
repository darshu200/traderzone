...
async def watchlist(asset_class: Optional[str] = None):
    ...
    # Bounded concurrency — 74 simultaneous NSE requests from one shared
    # session is exactly the pattern that gets a session rate-limited/blocked.
    # Reuses the same semaphore as the live scanner for consistency.
    async def _row_bounded(s):
        async with _SCAN_CONCURRENCY:
            return await asyncio.to_thread(_row, s)

    result = await asyncio.gather(*[_row_bounded(s) for s in active])
    return list(result)
